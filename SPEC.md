...
通过校准的 Judge 结果首先用于 Finding/报告排序和人工复核；第 6 阶段不得反向改变 Coverage、Corpus
或 Seed Energy。第 7 阶段门禁通过后，只有显式标记、版本锁定的 Judge 结果才可作为调度或 Corpus
取舍的次级信号，且不能单独创建事实覆盖或晋升种子。Judge 与执行事实冲突时保留执行事实，将评分
标记为 `provisional` 并进入人工复核。漂移监控达到锁定最小统计窗口且越过冻结阈值时自动冻结结论
发布和依赖 Judge 的调度；样本不足只告警，不宣称已确认漂移。恢复必须经人工确认或重新校准。纯执行
证据驱动的 Fuzzing 可以带降级标记继续，原始轨迹与覆盖数据不得删除。
...
第 6-7 阶段在前五阶段稳定前冻结：现在不建设通用评分器、rubric、黄金集、置信度、漂移阈值或
主动学习接口。固定案例的 utility/security 结果只使用确定性状态断言，不属于 LLM 裁判。
