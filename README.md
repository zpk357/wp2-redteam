...
Stage 9.1 已新增 Office V2 专用 `V2CoverageInput`。direct、recording 和 strict replay 三种采集方式
现在会汇入同一组行为事实与 Oracle 事实；采集来源和 lineage 保留，但不进入 canonical fact digest。
初始化 materialization 与 Agent 执行产生的状态转换保持分离。不完整录制、未匹配重放、未确认容器
清理、非 submit 终止或证据闭包不一致均不能进入覆盖计算。旧 `CoverageInputResolver` 继续隔离为 V1
资产，不作为 V2 输入。覆盖施工 `2.0` 已进一步冻结 V2 专用合同身份、4 个 RiskFamily 的主调度方向与
多风险 facet、blocked/realized 独立累计位、Utility 伴随事实和同批候选共享 baseline；旧 Store 的逐条
立即累计和单风险深度语义未接入 V2。`2.1` 已冻结 V2 一级行为特征、二级多样性、证据血缘和有界路径
规范化：实例 ID、正文、时间、cursor 与采集来源不会制造一级新颖度，真实顺序变化仍可区分，重复调用
和循环按 `1 / 2 / 3+` 折叠。`2.2` 已能从真实可信工具交换产出工具一至三元路径、跨业务域边、去值
参数形状与来源链、四层权限和结果/事务分支；每个特征保留 EvidenceRef，direct、recording 和 strict
replay 不会制造三份覆盖。`2.3` 已继续提取 committed StateDelta、可信交互和 submit 终止，初始化
overlay 与 Agent 副作用保持分离，并组装完整 `V2BehaviorProfile`。当前仍未形成风险 CoverageDelta；
下一项是 `2.4` 从冻结 Stage 5/6 资产编译风险目录和固定分母。
...
Ollama、LangGraph Agent、办公工具和场景状态；Qwen 自主选择工具及参数并真实改变容器内状态。
Controller 只负责生命周期、证据、双覆盖率、Corpus 和调度，不能替 Agent 规划工具序列。该拓扑的
本机完整办公链路已通过路线图 `5.G4`；GPU 服务器阶段门和 12 组合真实基线仍待 `5.G5-5.G6`。
...
本机历史检查点和真实模型归档领先于 GitHub，不能以远端版本覆盖；当前施工状态应以
本地 Git 检查点、`HANDOFF.md` 和 `LOG-INDEX.md` 共同确认。
...
- Office V2 第六步真实模型连续反馈 Campaign 计划已经重新冻结：正式 Agent 与独立 Mutator 使用
  `qwen3.5:27b-q4_K_M`，串行占用 RTX 4090 24GB，初始 context 为 8192。模型不适配或显存不足时阻塞，
  不自动降级；48 个冻结案例在本地检查，服务器先打通 2 代，再让同一个 Campaign 按覆盖收益恢复到
  20、30 或最多 50 代，不重复运行独立 24+24 付费矩阵。旧 `qwen3:8b` 包只作历史证据。详细门禁见
...
所有请求默认且只能使用 `trace_react_v2`。TRACE-G 自己维护消息历史、模型轮次、工具调用账本和
终止条件。工具执行结果必须追加为下一轮模型消息；只有一次有效 `submit` 才成功，轮次耗尽返回
`agent_no_submit`。
...
公平调度和可恢复 Campaign Store 已实现并通过本地聚焦验收；真实第二步 Coverage 到下一轮调度的
持久闭环也已贯通。语义候选生成和真实执行闭环尚未实现，
因此当前没有公开的 `coverage`、`mutate` 或 `campaign` 命令。旧模块
保留但不能消费 V2 结果，也不能作为正式产品入口。下一阶段只能消费真实工具调用/结果、PolicyDecision、
StateDelta、来源证据、Oracle 和终止事实；模型自报标签、单纯措辞差异、固定工具序列和 Judge 输出
不能作为覆盖事实。
